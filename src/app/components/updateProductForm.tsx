...
export default function UpdateProductForm() {
  ...
  const onProductUpdateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/product/${formData.get("product_id")}`,
      {
        method: "PATCH",
        body: formData,
        credentials: "include",
    ...
    <div className="w-1/3">
      <form onSubmit={onProductUpdateSubmit}>
        <div className="flex flex-col gap-3">
          ...
          <input type="text" name="description" className="bg-amber-200" />
          <label htmlFor="stock">Stock</label>
          <input type="number" name="stock" className="bg-amber-200" />
